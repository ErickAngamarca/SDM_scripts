###################### Download modelling data script  #########################
...
#    Vector download
#    Altitude download
#    Bio-climatic download 30''
#    Bio-climatic download 2,5'
...
#libraries
pacman::p_load(raster, sf, kuenm, tidyverse, datasets, geodata, nngeo, terra, 
               vegdistmod, rpaleoclim, ellipsenm)
...
## 5. Download countries shape file    #########################################
...
## 7. WorldClim bio-climatic variables 0.5'    #################################

#load data
mask_buff <- st_read("2_vector/countries_buff_4326.shp")

#download tiles
bio_30s_tile1 <- worldclim_tile(var = "bio", -75, -10, 
                                path = "3_raster/download/", version = "2.1")
bio_30s_tile2 <- worldclim_tile(var = "bio", -75, 10, path = "3_raster/download/", 
                                version = "2.1")

#load tiles
bio_30s_tile1 <- rast(paste0("3_raster/download/climate/wc2.1_tiles/", 
                             "tile_40_wc2.1_30s_bio.tif"))[[-c(10, 11, 18, 19)]]
bio_30s_tile2 <- rast(paste0("3_raster/download/climate/wc2.1_tiles/", 
                             "tile_28_wc2.1_30s_bio.tif"))[[-c(10, 11, 18, 19)]]
...
#update names
names(bio_30s_mask) <- c("bio_01", paste0("bio_", seq(10, 17)), 
...
wr <- lapply(1:nlayers(bio_30s_mask), function(x) {
  writeRaster(bio_30s_mask[[x]], filename = names_30s_wc[x], 
              format = "ascii", overwrite=T)
})

## 8. WorldClim bio-climatic variables 2.5'   ##################################
...
#bioclimatic variables download
{
  options(timeout = max(1000, getOption("timeout"))) 
...
## 9. WorldClim lgm bio-climatic variables 0.5'   ##############################
...
## 10. Chelsa bio-climatic variables 0.5'   ####################################
...
## 12. Chelsa lgm bio-climatic variables 2.5'  #################################
