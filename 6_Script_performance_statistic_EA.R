...
#-------------------------------------------------------no editing
# Extraer las abreviaturas usando stringr
...
# Extraer el p-valor y formatearlo
...
# Filtrar los datos donde 'product' es 'Chelsa'
df_chelsa <- df_partial_roc %>% filter(product == "Chelsa")

# Filtrar los datos donde 'product' es 'WorldClim'
...
plot_2 <- ggplot(df_combined, aes(x = product, y = Model_partial_AUC, fill = product)) +
  ...
  scale_y_continuous(labels = label_number(decimal.mark = ",")) +  # Eje Y con comas
  theme(
    panel.grid.major = element_blank(),   
    panel.grid.minor = element_blank(),   
    panel.border = element_rect(color = "black", linewidth = 0.5),  
  ...
  annotate("text", x = "Chelsa", y = 0.16, 
           label = gsub("\\.", ",", Wilcox_p_value),  # Comas en el p-valor
